...
while IFS= read -r proxy
	do
		{ echo -n $current_date_time ; echo -n ',' ; echo  $(curl -x $proxy -U "david.pinilla":"|Jn 5DJ\\7inbNniK|m@^ja&>C" -m 180 -w %{time_total},%{http_code},%{size_download} -H Cache-control:private http://ovh.net/files/1mb.dat -o /dev/null -s) ;} >>$proxy"_1gb"
		sleep 10s
		...
		sleep 10s
		{ echo -n $current_date_time ; echo -n ',' ; echo  $(curl -x $proxy -U "david.pinilla":"|Jn 5DJ\\7inbNniK|m@^ja&>C" -m 180 -w %{time_total},%{http_code} -H Cache-control:private http://ipv4.download.thinkbroadband.com/5MB.zip -o /dev/null -s) ;} >>$proxy_"1gb"
		sleep 10s
